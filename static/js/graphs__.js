...
function makeGraphs(error, votesJson, statesJson) {
  ...
  var reducer = reductio().max(function(d) { return parseInt(d["nb_votes"])}); // + ":" + d["vote"]; });


  //console.log(reducer(group).top(5))
  var group2 = stateDim.group();
  var reducer2 = reductio().max(function(d) { return parseInt(d["nb_votes"])})
                           .value("Candidate").exception(function(d) { return parseInt(d["vote"])});
